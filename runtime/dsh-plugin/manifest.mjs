 ...
 * 不做 TS schema 的完整校验（运行时只读关键字段），关键不一致在 mountAgentScope/工具执行时再校验。
 */
export function loadManifest(cwd, sessionId) {
  ...
  if (m.sessionId !== sessionId) {
    throw new ManifestError("DSH_MANIFEST_INVALID", "manifest sessionId 与请求不一致");
  }
  if (m.kind !== "persona" && m.kind !== "moderator") {
    throw new ManifestError("DSH_MANIFEST_INVALID", "manifest kind 非法");
  }
  if (!m.toolPolicy || m.toolPolicy.sideEffects !== false) {
    throw new ManifestError("DSH_MANIFEST_INVALID", "manifest 必须声明 sideEffects=false");
  }
  return m;
}

/** 读取并验证 snapshot/packageRoot 下某文件的 sha256；与 manifest 中声明 hash 一致才返回 */
export function readVerifiedFile(rootDir, relPath, sha256Hex, cwd, sizeLimit = MAX_REFERENCE_BYTES) {
...
export function resolveEntryRoot(cwd, manifest, entry) {
  ...
  if (!isSafeReferenceRoot(entry.packageRoot)) {
    throw new ManifestError("DSH_MANIFEST_INVALID", `Skill ${entry.name} packageRoot 非法`);
  }
  return resolveRoot(cwd, entry.packageRoot);
}

/** packageRoot/snapshotRoot → 绝对路径 + realpath 边界校验（data/skill-library / data/dsh/snapshots 内） */
export function resolveRoot(cwd, root) {
