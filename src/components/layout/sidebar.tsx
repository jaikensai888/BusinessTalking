...
export function Sidebar({ collapsed, onClose }: { collapsed: boolean; onClose: () => void }) {
  ...
  const renderItem = (it: { href: string; label: string; icon: Icon }) => {
    ...
    return (
      <Link
        ...
        className={cn(
          "relative flex items-center gap-3 rounded-lg px-3 py-2.5 text-caption font-semibold transition-colors duration-150 whitespace-nowrap",
          active
  ...
  return (
    <>
      {/* ≤833px 时侧边栏浮为抽屉，点击遮罩收起（规范：此断点起导航折叠） */}
      {!collapsed && (
        <div
          aria-hidden="true"
          onClick={onClose}
          className="fixed inset-0 z-30 hidden bg-black/40 max-[833px]:block"
        />
      )}
      <aside
        className={cn(
          ...
          "max-[833px]:absolute max-[833px]:z-40 max-[833px]:h-full max-[833px]:shadow-float",
      ...
      >
        ...
        <div className={cn("mt-auto flex flex-col gap-1 px-3 pb-5", collapsed && "hidden")}>
          ...
          {renderItem(SETTINGS)}
          <p className="px-3 pt-2 text-fine text-ink-40">v0.1 · BusinessTalking</p>
        </div>
      </aside>
    </>
  );
