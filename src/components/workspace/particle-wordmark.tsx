...
export function ParticleWordmark() {
  ...
  useEffect(() => {
    ...
    const init = (w: number, h: number) => {
      ...
      // 环绕的漂浮光点（普通混合，浅色背景上也清晰可见）
      count = Math.max(80, Math.round((W * H) / 260));
      ...
      mat = new THREE.ShaderMaterial({
        ...
        fragmentShader: `
          ...
          void main() {
            ...
            float twinkle = 0.6 + 0.4 * sin(uTime * 2.4 + vPhase);
            gl_FragColor = vec4(uColor, disk * twinkle);
          }
        `,
      });
    ...
    const paint = () => {
      ...
      for (let i = 0; i < count; i++) {
        ...
        const ph = phases[i] ?? 0;
        let x = pos[i3] + Math.sin(t * sp + ph) * 6;
        let y = pos[i3 + 1] + Math.cos(t * sp * 0.9 + ph * 1.6) * 6;
        const z = pos[i3 + 2] + Math.sin(t * 0.6 + ph) * 1.4;
        if (!reduced && pointer.active) {
          const dx = x - pointer.x;
          const dy = y - pointer.y;
          const distance = Math.sqrt(dx * dx + dy * dy);
          if (distance < radius && distance > 0.01) {
            const force = ((radius - distance) / radius) ** 2 * 22;
            x += (dx / distance) * force;
            y += (dy / distance) * force;
          }
        }
  ...
  return (
    <div ref={wrapRef} className="relative mx-auto flex h-[132px] w-[620px] max-w-[94vw] items-center justify-center">
