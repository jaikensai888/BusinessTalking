...
/** 生成 BT runtime overlays（file:// 绝对路径；相对路径在 pnpm 布局的 dsh 运行时下无法装载） */
export function ensurePluginPatch(cwd: string): string {
  ...
  const resumePath = path.join(cwd, "runtime", "dsh-session-resume", "index.mjs");
  const resumeUrl = pathToFileURL(resumePath).href;
  const patch = `# generated by dsh/singleton.ts — BT runtime overlays\n- insert:\n    - id: business-talking\n      name: '${url}'\n    - id: business-talking-session-resume\n      name: '${resumeUrl}'\n      inject: [agents, sessionQuery]\n`;
